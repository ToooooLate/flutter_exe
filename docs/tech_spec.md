...
- 开发环境：macOS，Windows 打包需在 Windows 环境进行。
...
- 配置文件：`desktop/assets/config/app.json`，包含目标网页地址与健康检查路径。
...
- 核心依赖
  ...
  - `http`：健康检查与目标主机可达验证。
- 启动流程
  - 读取 `assets/config/app.json`：`webBaseUrl`、`healthCheckPath`、`timeoutMs`。
  - 进行健康检查：HTTP GET `webBaseUrl + healthCheckPath`，设置超时和错误重试。
  - 健康检查通过：加载 `webBaseUrl` 到 `WebView`。
  - 健康检查失败：展示错误页（提示“网络不可用或服务不可达”）+ “重试”按钮。
- 关键逻辑（已实现）
  - `AppConfig`：配置读取与校验。
  - `StartupGate`：健康检查与状态页（loading/ok/error）。
  - `WebShell`：WebView 页面，限制同域导航。
- 配置示例（已内置）
  - `desktop/assets/config/app.json`：`{ "webBaseUrl": "http://127.0.0.1:5173/", "healthCheckPath": "health", "timeoutMs": 3000 }`
- Windows 打包与运行
...
- 初始化（已完成）
  - 已克隆至 `qingzhi/web/`。
  - 健康检查：新增静态文件 `public/health`，访问 `GET /health` 返回 200。
- 开发与构建
  - 安装：`cd web && npm i -g corepack && pnpm install`
  - 开发：`pnpm dev`（默认端口 5173）
  - 构建：`pnpm build`
- 二次开发（计划）
  - 新建业务模块：`engine`（仪表盘、列表、详情、图表、告警）。
  - 图表库：`echarts`（`echarts` + `vue-echarts`）。
  - 接口服务：`/api/engines`、`/api/engines/:id/metrics`、`/api/metrics/:id/timeseries?from=&to=`；开发期可先用 Mock。
...
## 开发流程（Trae 视角）
- 准备
  - 创建双项目目录：`qingzhi/desktop` 与 `qingzhi/web`（已完成）。
  - 为桌面项目新增配置文件与依赖；为前端初始化健康检查（已完成）。
- 桌面端实现
  - 已引入依赖：`webview_flutter connectivity_plus http`。
  - 编写启动页与错误页，接入配置与健康检查逻辑（已完成）。
  - 封装 `NavigationDelegate` 限制导航域名（已完成）。
- 前端实现（下一步）
  - 建立 `engine` 模块路由、菜单与页面骨架。
  - 接入 `echarts` 图表与接口服务层，先用 Mock 数据联调。
  - 配置更丰富的 `/health` 响应，确保 Flutter 端可达。
- 联调与验收
...
- 升级与配置
  - 通过修改 `desktop/assets/config/app.json` 更新目标前端地址，无需重新编译。
...
## 里程碑与交付
- M1：Flutter 桌面端骨架与网络检测页；Vben Admin 初始化与健康检查接口（已完成部分）。
- M2：前端 `engine` 模块页面骨架与图表联调；桌面端导航限制与错误页完善。
- M3：接口联通与性能优化；打包与分发流程固化；验收测试通过。

## 参考与备注
- Vben Admin v5 项目主页：Vue 3、Vite、TypeScript、主题、国际化、权限、动态路由等。地址：`https://github.com/vbenjs/vue-vben-admin`。
